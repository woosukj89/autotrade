"""
Idea A test: graduated (not binary) allocation using this session's
calibrated multi-signal vote, run through the real 20yr backtest, compared
against every baseline established so far:
  - HighBetaOnly: the stock-picker alone, zero timing (new strongest
    baseline found: 26.8% CAGR / 64.7% MaxDD - see test_diversification.py)
  - RegimeAdaptive (MacroMom, live, unmodified): 25.1% / 66.1%
  - Binary[rolling126_10]: best binary classifier strategy: 23.7% / 47.2%
  - Graduated[<definition>] for each of the 7 bear definitions' calibrated
    classifiers, using the DEFAULT (concentrated) stock-picker settings -
    diversification alone wasn't a good trade (test_diversification.py),
    so isolate the graduated-exposure effect on the strong baseline first.
"""
import json
import os
import sys
...
from graduated_strategy import GraduatedClassifierStrategy
...
def main():
    ...
    for name in def_names:
        ...
        strategy = GraduatedClassifierStrategy(params, db_path=db_path, data=clf_data, max_positions=15)
        results[f'Graduated[{name}]'] = run_backtest(f'Graduated[{name}]', strategy, bt, args.years, args.end_year)

    print(f"\n{'='*100}\nGRADUATED COMPARISON ({args.years}yr, after {args.tax_rate*100:.0f}% tax, ending {args.end_year})\n{'='*100}")
    ...
    for name, result in results.items():
        if result and result.snapshots:
            ...
            df.to_csv(os.path.join(RESULTS_DIR, f'graduated_{slug}_{args.years}yr.csv'), index=False)
    with open(os.path.join(RESULTS_DIR, f'graduated_comparison_{args.years}yr.json'), 'w') as f:
        json.dump(out, f, indent=2)
    print(f"\nSaved results/graduated_comparison_{args.years}yr.json + per-strategy CSVs")
